...
const GitHubStats: React.FC = () => {
  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      try {
        const userResponse = await githubService.getUserData('SaiRamReddySolleti');
        setUserData(userResponse.data as UserData);

        const reposResponse = await githubService.getRepositories('SaiRamReddySolleti', { per_page: 100 });
